...
export function useWorkouts() {
  ...
  const fetchLogsForType = useCallback(
    async (
      ...
      const results = await Promise.all(
        type.exercises.map((e) => workoutsApi.listLogs(e.id)),
      );
  ...
  const createExercise = useCallback(
    async (typeId: string, name: string): Promise<Exercise> => {
      const sortOrder =
        types.find((t) => t.id === typeId)?.exercises?.length ?? 0;
      const exercise = await workoutsApi.createExercise(
        typeId,
        name,
        sortOrder,
      );
  ...
  const deleteExercise = useCallback(
    async (typeId: string, id: string): Promise<void> => {
      await workoutsApi.deleteExercise(id);
      setTypes((prev) =>
        prev.map((t) =>
          t.id === typeId
            ? { ...t, exercises: (t.exercises ?? []).filter((e) => e.id !== id) }
            : t,
        ),
      );
  ...
  const upsertLog = useCallback(
    async (
      ...
      const log = await workoutsApi.upsertLog(
        exerciseId,
        weekNumber,
        sets,
        reps,
